...
void Window::change_func() { set_function((func_id + 1) % 7); }
...
void Window::paintEvent(QPaintEvent * /* event */) {
    ...
    bool params_changed = !cache_valid || cache_a != a || cache_b != b ||
                          cache_n != n || cache_k != func_id ||
                          cache_perturbation != perturbation ||
                          cache_scale != scale;
    ...
    for (double x = a_scaled; x <= b_scaled; x += delta_x) {
        ...
        case 3:
            if (is_first_method) {
                value = fabs(
                    EvaluationMethod11(x, a_scaled, b_scaled, n, X, F, A1) -
                    f(x));
                min_y = GetMin(min_y, value, first_p, 1);
                max_y = GetMax(max_y, value, first_p, 1);
                first_p = false;
            ...
            if (is_second_method) {
                value = fabs(
                    EvaluationMethod36(x, a_scaled, b_scaled, n, X, F, A2) -
                    f(x));
                min_y = GetMin(min_y, value, first_p, 1);
                max_y = GetMax(max_y, value, first_p, 1);
                first_p = false;
    ...
    case 0:
        ...
        if (is_first_method) {
            DrawingApproximation(painter, a_scaled, b_scaled, delta_x, n, X, F,
                                 A1, 1);
        }
        break;
    case 1:
    ...
    if (display == 3) {
        ...
        if (is_second_method) {
            ...
            painter.drawText(
                10, pos, QString("residual (method 36) = %1").arg(residual));
            pos += 20;
        }
    }
